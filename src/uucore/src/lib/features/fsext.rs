...
impl MountInfo {
    ...
    fn new(volume_name_buf: &[u16]) -> Option<Self> {
        ...
        .map(|info| info.characteristics & nt::FILE_REMOTE_DEVICE != 0)
        .unwrap_or(false);

        let mount_dir: String = unsafe {
            let mut buf = [MaybeUninit::uninit(); MAX_PATH as usize];
            let mut len = 0u32;
            ...
            buf[..len as usize]
                ...
                .fold(String::new(), |mut acc, str| {
                    ...
                    acc.extend(
                        char::decode_utf16(str.iter().cloned())
...
pub fn read_fs_list() -> UResult<Vec<MountInfo>> {
    ...
    #[cfg(windows)]
    {
        ...
        let find_handle = unsafe {
            FindFirstVolumeW(
                volume_name_buf.as_mut_ptr().cast(),
...
// spell-checker:disable
const FSTYPE: [(i64, &'static str); 117] = [
    (0x6163_6673, "acfs"),
    (0xADF5, "adfs"),
    (0xADFF, "affs"),
    (0x5346_414F, "afs"),
